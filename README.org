...
When =org-window-habit-show-streak= is non-nil, the graph also appends a
compact =S<n>= indicator. This is the current streak: the number of
consecutive assessment intervals, ending with the current interval, whose
aggregate conforming ratio is at least =org-window-habit-streak-threshold=.
For a weekly 5-of-7 habit with a weekly assessment interval, =S3= means
three consecutive conforming weeks. For a rolling daily assessment, it means
consecutive conforming days.
...
| =org-window-habit-show-streak=                   | =t=         | Show current streak after graph    |
| =org-window-habit-streak-format=                 | =" S%d"=    | Format for the streak indicator    |
| =org-window-habit-streak-threshold=              | =1.0=       | Ratio required to extend streak    |
