...
4. Ensure Org is logging TODO state changes for the entry. =org-window-habit=
   reads completion history from standard Org state-change logs whether they
   are stored inline or inside a =LOGBOOK= drawer. Setting
   =org-log-into-drawer= is optional.
...
| =:auto-complete-fn=     | No       | (none)        | Predicate for optional automatic completion        |
...
Some habits can be completed from external state instead of manual action. Add
=:auto-complete-fn= to a habit's =OWH_CONFIG= to opt that habit into automatic
completion:

#+begin_src org
,* TODO Take morning medication
SCHEDULED: <2024-01-15 Mon .+1d>
:PROPERTIES:
:STYLE: habit
:OWH_CONFIG: (:window-specs ((:duration (:days 1) :repetitions 1)) :auto-complete-fn my-medication-taken-p)
:END:
#+end_src
...
#+begin_src emacs-lisp
(funcall auto-complete-fn marker habit now)
#+end_src

It should return non-nil when the entry should be completed. The completion
itself is still performed by org-window-habit so logging, duplicate checks, and
rescheduling stay centralized.

Automatic completion can be run manually:

#+begin_src emacs-lisp
(org-window-habit-auto-complete-run)
#+end_src

Or enabled with a global timer:

#+begin_src emacs-lisp
(org-window-habit-auto-complete-mode +1)
#+end_src

The mode schedules one timer at a time. Each timer run scans the current
=org-agenda-files=, completes eligible habits, then schedules the next run with
=org-window-habit-auto-complete-interval= plus fresh random jitter from
=org-window-habit-auto-complete-jitter=. This means habits added after startup
are picked up on the next scan as long as they are in an agenda file.

To reduce cross-machine races, a run skips a habit that already has a completion
in the current assessment interval, and it repeats that check immediately before
mutating the buffer. If any agenda file buffer has unsaved edits, the run
aborts rather than saving unrelated user changes.

For git-synchronized org files, you can configure a conservative prefetch:

#+begin_src emacs-lisp
(setq org-window-habit-auto-complete-prefetch-function
      #'org-window-habit-auto-complete-prefetch-git-ff-only)
#+end_src

The built-in prefetch helper infers the single git repository containing your
agenda files, refuses dirty worktrees, fetches, and only advances via
fast-forward.
...
*Example: Averaging multiple windows*
#+begin_src org
:PROPERTIES:
:OWH_CONFIG: (:window-specs ((:duration (:days 2) :repetitions 1 :value 1.0)
                             (:duration (:days 4) :repetitions 2 :value 1.0)
                             (:duration (:days 8) :repetitions 4 :value 1.0))
             :assessment-interval (:days 1)
             :aggregation-fn org-window-habit-weighted-average-aggregation-fn)
:END:
#+end_src

This averages the completion ratios for the 2, 4, and 8 day windows instead of
taking the minimum across them.
...
| =org-window-habit-auto-complete-interval=        | =1800=      | Base seconds between auto scans    |
| =org-window-habit-auto-complete-jitter=          | =300=       | Maximum random seconds per scan    |
| =org-window-habit-auto-complete-prefetch-function= | =nil=     | Function run before auto scans     |
| =org-window-habit-auto-complete-after-save-function= | =nil=  | Function run after auto saves      |
